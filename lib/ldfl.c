...
#define ldfl_variadic_str_wrap(target_func, nvarg, ...)                                                                                              \
    ({                                                                                                                                               \
        ...
        va_end(va_list_name);                                                                                                                        \
                                                                                                                                                     \
        ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_CRIT, "call '%s', variadic arg count too high: %d (limit: 8)", #target_func, _arg_count);          \
        ...
        case 6:                                                                                                                                      \
            ...
            break;                                                                                                                                   \
        case 7:                                                                                                                                      \
            _ret = target_func(__VA_ARGS__, _args[0], _args[1], _args[2], _args[3], _args[4], _args[5], _args[6], NULL);                             \
            break;                                                                                                                                   \
        case 8:                                                                                                                                      \
            _ret = target_func(__VA_ARGS__, _args[0], _args[1], _args[2], _args[3], _args[4], _args[5], _args[6], _args[7], NULL);                   \
            break;                                                                                                                                   \
        default:                                                                                                                                     \
            ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_CRIT, "call '%s', variadic arg count too high: %d (limit: 8)", #target_func, _arg_count);      \
...
bool ldfl_find_matching_rules(const char *call, const char *pathname, uint64_t mask, compiled_mapping_t ***return_rules, int *num_rules,
                              pcre2_match_data ***return_pcre_match) {
    if (pathname == NULL) {
        return false;
    }

    int matching_rule_count = 0;

    // FIXME somewhat suboptimal allocation
    // We should not allocate that much (it's the worst case if all rules match)
    *return_rules      = calloc(sizeof(compiled_mapping_t *), ldfl_rule_count);
    *return_pcre_match = calloc(sizeof(pcre2_match_data *), ldfl_rule_count);
    for (int i = 0; i < ldfl_rule_count; i++) {
        // Rule not matching
        if (!(ldfl_compiled_rules[i].mapping->operation & mask) || (ldfl_compiled_rules[i].matching_regex == NULL)) {
            ldfl_setting.logger(LDFL_LOG_MAPPING_RULE_SEARCH, LOG_DEBUG, "rule[%s] not relevant for call '%s', skipping", ldfl_mapping[i].name, call);
            continue;
        }

        pcre2_match_data *match_data = pcre2_match_data_create_from_pattern(ldfl_compiled_rules[i].matching_regex, NULL);
        ...
        if (rc <= 0) {
            ldfl_setting.logger(LDFL_LOG_MAPPING_RULE_SEARCH, LOG_DEBUG, "rule[%s] not matching pathname '%s' for call '%s'", ldfl_mapping[i].name,
                                pathname, call);
        ...
        ldfl_setting.logger(LDFL_LOG_MAPPING_RULE_FOUND, LOG_INFO, "rule[%s] match pathname '%s', selected for call '%s'", ldfl_mapping[i].name,
                            pathname, call);
        matching_rule_count++;
    }
...
static char *ldfl_substitute_path(pcre2_code *regex, pcre2_match_data *match_data, const char *pathname, const char *target) {
    ...
    int rc = pcre2_substitute(regex, (PCRE2_SPTR)pathname, PCRE2_ZERO_TERMINATED, 0, PCRE2_SUBSTITUTE_GLOBAL, match_data, NULL, (PCRE2_SPTR)target,
...
void ldfl_apply_rules(compiled_mapping_t **mapping_rules, int num_rules, pcre2_match_data **match_group, const char *pathname_in,
                      char **pathname_out) {
    if (pathname_in == NULL) {
    ...
    if (num_rules <= 0) {
        ldfl_setting.logger(LDFL_LOG_MAPPING_RULE_APPLY, LOG_DEBUG, "No Rule to apply on path '%s', returning the same path", pathname_in);
    ...
    for (int i = 0; i < num_rules; i++) {
        ...
        case LDFL_OP_NOOP:
            *pathname_out = calloc(sizeof(char), strlen(pathname_in) + 1);
            ...
            break;
        case LDFL_OP_MAP:
            new_pathname = ldfl_substitute_path(mapping_rules[i]->matching_regex, match_group[i], pathname_in, mapping_rules[i]->mapping->target);
            if (!new_pathname) {
                ldfl_setting.logger(LDFL_LOG_MAPPING_RULE_APPLY, LOG_WARNING, "Replacement in path failed for rule '%s' on path '%s'",
            ...
            *pathname_out = new_pathname;
            ...
            ldfl_setting.logger(LDFL_LOG_MAPPING_RULE_APPLY, LOG_DEBUG, "LDFL_OP_MAP Rule [%s] applied, path '%s' rewritten to '%s'",
                                mapping_rules[i]->mapping->name, pathname_in, *pathname_out);
            break;
        case LDFL_OP_EXEC_MAP:
            new_pathname = ldfl_substitute_path(mapping_rules[i]->matching_regex, match_group[i], pathname_in, mapping_rules[i]->mapping->target);
            if (!new_pathname) {
                ldfl_setting.logger(LDFL_LOG_MAPPING_RULE_APPLY, LOG_WARNING, "Replacement in executable path failed for rule '%s' on path '%s'",
                                    mapping_rules[i]->mapping->name, pathname_in);
                *pathname_out = NULL;
                return;
            }
            *pathname_out = new_pathname;
            ...
            ldfl_setting.logger(LDFL_LOG_MAPPING_RULE_APPLY, LOG_DEBUG, "LDFL_OP_EXEC_MAP Rule [%s] applied, executable path '%s' rewritten to '%s'",
                                mapping_rules[i]->mapping->name, pathname_in, *pathname_out);
            break;
...
int (*real_statx)(int dirfd, const char *restrict pathname, int flags, unsigned int mask, struct statx *restrict statxbuf);
...
FILE *fopen(const char *restrict pathname, const char *restrict mode) {
    ...
    LDFL_LOG_ERR(ret, "real_fopen failed: pathname=%s, mode=%s, errno=%d (%s)", reworked_path, mode, errno, strerror(errno));
...
FILE *fopen64(const char *pathname, const char *mode) {
    ...
    LDFL_LOG_ERR(ret, "real_fopen64 failed: pathname=%s, mode=%s, errno=%d (%s)", reworked_path, mode, errno, strerror(errno));
...
int openat(int dirfd, const char *pathname, int flags, ...) {
    ...
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "openat called: dirfd=%d, pathname=%s, flags=%d, mode=%o", dirfd, pathname, flags,
                        va_arg(args, mode_t));
    ...
    LDFL_LOG_ERR(ret, "real_openat failed: dirfd=%d, pathname=%s, flags=%d, errno=%d (%s)", dirfd, reworked_path, flags, errno, strerror(errno));
...
int open(const char *pathname, int flags, ... /* mode_t mode */) {
    ...
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    // FIXME handle variadic properly
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "open called: pathname=%s, flags=%d, mode=%o", pathname, flags, va_arg(args, mode_t));
    ...
    LDFL_LOG_ERR(ret, "real_open failed: pathname=%s, flags=%d, errno=%d (%s)", reworked_path, flags, errno, strerror(errno));
...
int open64(const char *pathname, int flags, ... /* mode_t mode */) {
    ...
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    // FIXME handle variadic properly
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "open64 called: pathname=%s, flags=%d, mode=%o", pathname, flags, va_arg(args, mode_t));
    ...
    LDFL_LOG_ERR(ret, "real_open64 failed: pathname=%s, flags=%d, errno=%d (%s)", reworked_path, flags, errno, strerror(errno));
...
int openat64(int dirfd, const char *pathname, int flags, ... /* mode_t mode */) {
    ...
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    // FIXME handle variadic properly
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "openat64 called: dirfd=%d, pathname=%s, flags=%d, mode=%o", dirfd, pathname, flags,
                        va_arg(args, mode_t));
    ...
    LDFL_LOG_ERR(ret, "real_openat64 failed: dirfd=%d, pathname=%s, flags=%d, errno=%d (%s)", dirfd, reworked_path, flags, errno, strerror(errno));
...
int rename(const char *oldpath, const char *newpath) {
    ...
    LDFL_LOG_ERR(ret, "real_rename failed: oldpath=%s, newpath=%s, errno=%d (%s)", reworked_oldpath, reworked_newpath, errno, strerror(errno));
...
int renameat2(int olddirfd, const char *oldpath, int newdirfd, const char *newpath, unsigned int flags) {
    ...
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    REAL(renameat2);
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "renameat2 called: olddirfd=%d, oldpath=%s, newdirfd=%d, newpath=%s, flags=%u", olddirfd,
...
int renameat(int olddirfd, const char *oldpath, int newdirfd, const char *newpath) {
    RINIT;
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "renameat called: olddirfd=%d, oldpath=%s, newdirfd=%d, newpath=%s", olddirfd, oldpath, newdirfd,
                        newpath);
    ...
    LDFL_LOG_ERR(ret, "real_renameat failed: olddirfd=%d, oldpath=%s, newdirfd=%d, newpath=%s, errno=%d (%s)", olddirfd, reworked_oldpath, newdirfd,
...
int unlinkat(int dirfd, const char *pathname, int flags) {
    RINIT;
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "unlinkat called: dirfd=%d, pathname=%s, flags=%d", dirfd, pathname, flags);
    char *reworked_path = apply_rules_and_cleanup("unlinkat", pathname, op_mask);

    int ret = real_unlinkat(dirfd, reworked_path, flags);
    LDFL_LOG_ERR(ret == 0, "real_unlinkat failed: dirfd=%d, pathname=%s, flags=%d, errno=%d (%s)", dirfd, reworked_path, flags, errno,
                 strerror(errno));
...
int utime(const char *pathname, const struct utimbuf *times) {
    RINIT;
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "utimes called: pathname=%s, times=[%ld, %ld]", pathname, (times == NULL) ? 0 : times->actime,
...
int utimes(const char *pathname, const struct timeval times[2]) {
    ...
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "utimes called: pathname=%s, times=[%ld:%ld, %ld:%ld]", pathname,
                        (times == NULL) ? 0 : times[0].tv_sec, (times == NULL) ? 0 : times[0].tv_usec, (times == NULL) ? 0 : times[1].tv_sec,
                        (times == NULL) ? 0 : times[1].tv_usec);
...
int utimensat(int dirfd, const char *pathname, const struct timespec times[2], int flags) {
    RINIT;
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "utimensat called: dirfd=%d, pathname=%s, times=[%ld, %ld], flags=%d", dirfd, pathname,
    ...
    LDFL_LOG_ERR(ret == 0, "real_utimensat failed: dirfd=%d, pathname=%s, flags=%d, errno=%d (%s)", dirfd, pathname, flags, errno, strerror(errno));
...
int access(const char *pathname, int mode) {
    ...
    LDFL_LOG_ERR(ret == 0, "real_access failed: pathname=%s, mode=%d, errno=%d (%s)", reworked_path, mode, errno, strerror(errno));
...
int fstatat(int dirfd, const char *pathname, struct stat *statbuf, int flags) {
    RINIT;
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "fstatat called: dirfd=%d, pathname=%s, flags=%d", dirfd, pathname, flags);
    ...
    LDFL_LOG_ERR(ret == 0, "real_fstatat failed: dirfd=%d, pathname=%s, flags=%d, errno=%d (%s)", dirfd, pathname, flags, errno, strerror(errno));
...
int __xstat(int version, const char *pathname, struct stat *statbuf) {
    ...
    LDFL_LOG_ERR(ret == 0, "real___xstat failed: version=%d, pathname=%s, errno=%d (%s)", version, pathname, errno, strerror(errno));
...
int __xstat64(int version, const char *pathname, struct stat *statbuf) {
    ...
    LDFL_LOG_ERR(ret == 0, "real___xstat64 failed: version=%d, pathname=%s, errno=%d (%s)", version, pathname, errno, strerror(errno));
...
int __lxstat(int version, const char *pathname, struct stat *statbuf) {
    ...
    LDFL_LOG_ERR(ret == 0, "real___lxstat failed: version=%d, pathname=%s, errno=%d (%s)", version, pathname, errno, strerror(errno));
...
int __fxstatat(int version, int dirfd, const char *pathname, struct stat *statbuf, int flags) {
    RINIT;
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "__fxstatat called: version=%d, dirfd=%d, pathname=%s, flags=%d", version, dirfd, pathname,
                        flags);
    ...
    LDFL_LOG_ERR(ret == 0, "real___fxstatat failed: version=%d, dirfd=%d, pathname=%s, flags=%d, errno=%d (%s)", version, dirfd, pathname, flags,
...
int execve(const char *pathname, char *const argv[], char *const envp[]) {
    ...
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "execve called: pathname=%s, argv=%s, envp=%s", pathname, argv_str, envp_str);
    free(argv_str);
    free(envp_str);
...
int mkdir(const char *pathname, mode_t mode) {
    ...
    LDFL_LOG_ERR(ret == 0, "real_mkdir failed: pathname=%s, mode=%d, errno=%d (%s)", reworked_path, mode, errno, strerror(errno));
...
int mkdirat(int dirfd, const char *pathname, mode_t mode) {
    RINIT;
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "mkdirat called: dirfd=%d, pathname=%s, mode=%o", dirfd, pathname, mode);
    char *reworked_path = apply_rules_and_cleanup("mkdirat", pathname, op_mask);

    int ret = real_mkdirat(dirfd, reworked_path, mode);
    LDFL_LOG_ERR(ret == 0, "real_mkdirat failed: dirfd=%d, pathname=%s, mode=%d, errno=%d (%s)", dirfd, reworked_path, mode, errno, strerror(errno));
...
int symlink(const char *target, const char *linkpathname) {
    RINIT;
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "symlink called: target=%s, linkpathname=%s", target, linkpathname);
    ...
    LDFL_LOG_ERR(ret == 0, "real_symlink failed: target=%s, linkpathname=%s, errno=%d (%s)", reworked_target, reworked_linkpathname, errno,
                 strerror(errno));
...
int link(const char *oldpath, const char *newpath) {
    ...
    LDFL_LOG_ERR(ret == 0, "real_link failed: oldpath=%s, newpath=%s, errno=%d (%s)", reworked_oldpath, reworked_newpath, errno, strerror(errno));
...
int linkat(int olddirfd, const char *oldpath, int newdirfd, const char *newpath, int flags) {
    RINIT;
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "linkat called: olddirfd=%d, oldpath=%s, newdirfd=%d, newpath=%s, flags=%d", olddirfd, oldpath,
...
int chmod(const char *pathname, mode_t mode) {
    ...
    LDFL_LOG_ERR(ret == 0, "real_chmod failed: pathname=%s, mode=%d, errno=%d (%s)", reworked_path, mode, errno, strerror(errno));
...
int truncate(const char *pathname, off_t length) {
    ...
    LDFL_LOG_ERR(ret == 0, "real_truncate failed: pathname=%s, length=%ld, errno=%d (%s)", reworked_path, length, errno, strerror(errno));
...
int faccessat(int dirfd, const char *pathname, int mode, int flags) {
    RINIT;
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "faccessat called: dirfd=%d, pathname=%s, mode=%d, flags=%d", dirfd, pathname, mode, flags);
    ...
    LDFL_LOG_ERR(ret == 0, "real_faccessat failed: dirfd=%d, pathname=%s, mode=%d, flags=%d, errno=%d (%s)", dirfd, reworked_path, mode, flags, errno,
                 strerror(errno));
...
int lchown(const char *pathname, uid_t owner, gid_t group) {
    ...
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "lchown called: pathname=%s, owner=%d, group=%d", pathname, owner, group);
    char *reworked_path = apply_rules_and_cleanup("lchown", pathname, op_mask);

    int ret = real_lchown(reworked_path, owner, group);
    LDFL_LOG_ERR(ret == 0, "real_lchown failed: pathname=%s, owner=%d, group=%d, errno=%d (%s)", reworked_path, owner, group, errno, strerror(errno));
...
int chown(const char *pathname, uid_t owner, gid_t group) {
    ...
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "chown called: pathname=%s, owner=%d, group=%d", pathname, owner, group);
    char *reworked_path = apply_rules_and_cleanup("chown", pathname, op_mask);

    int ret = real_chown(reworked_path, owner, group);
    LDFL_LOG_ERR(ret == 0, "real_chown failed: pathname=%s, owner=%d, group=%d, errno=%d (%s)", reworked_path, owner, group, errno, strerror(errno));
...
int fchmodat(int dirfd, const char *pathname, mode_t mode, int flags) {
    ...
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "fchmodat called: dirfd=%d, pathname=%s, mode=%o, flags=%d", dirfd, pathname, mode, flags);
    char *reworked_path = apply_rules_and_cleanup("fchmodat", pathname, op_mask);

    int ret = real_fchmodat(dirfd, reworked_path, mode, flags);
    LDFL_LOG_ERR(ret == 0, "real_fchmodat failed: dirfd=%d, pathname=%s, mode=%d, flags=%d, errno=%d (%s)", dirfd, reworked_path, mode, flags, errno,
                 strerror(errno));
...
int symlinkat(const char *target, int newdirfd, const char *linkpathname) {
    ...
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "symlinkat called: target=%s, newdirfd=%d, linkpathname=%s", target, newdirfd, linkpathname);
    ...
    LDFL_LOG_ERR(ret == 0, "real_symlinkat failed: target=%s, newdirfd=%d, linkpathname=%s, errno=%d (%s)", reworked_target, newdirfd,
...
int mkfifo(const char *pathname, mode_t mode) {
    ...
    LDFL_LOG_ERR(ret == 0, "real_mkfifo failed: pathname=%s, mode=%d, errno=%d (%s)", reworked_path, mode, errno, strerror(errno));
...
int mkfifoat(int dirfd, const char *pathname, mode_t mode) {
    ...
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "mkfifoat called: dirfd=%d, pathname=%s, mode=%o", dirfd, pathname, mode);
    char *reworked_path = apply_rules_and_cleanup("mkfifoat", pathname, op_mask);

    int ret = real_mkfifoat(dirfd, reworked_path, mode);
    LDFL_LOG_ERR(ret == 0, "real_mkfifoat failed: dirfd=%d, pathname=%s, mode=%d, errno=%d (%s)", dirfd, reworked_path, mode, errno, strerror(errno));
...
int mknodat(int dirfd, const char *pathname, mode_t mode, dev_t dev) {
    ...
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "mknodat called: dirfd=%d, pathname=%s, mode=%o, dev=%lu", dirfd, pathname, mode,
                        (unsigned long)dev);
    char *reworked_path = apply_rules_and_cleanup("mknodat", pathname, op_mask);

    int ret = real_mknodat(dirfd, reworked_path, mode, dev);
    LDFL_LOG_ERR(ret == 0, "real_mknodat failed: dirfd=%d, pathname=%s, mode=%d, dev=%ld, errno=%d (%s)", dirfd, reworked_path, mode, dev, errno,
                 strerror(errno));
...
int mknod(const char *pathname, mode_t mode, dev_t dev) {
    ...
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "mknod called: pathname=%s, mode=%o, dev=%lu", pathname, mode, (unsigned long)dev);
    char *reworked_path = apply_rules_and_cleanup("mknod", pathname, op_mask);

    int ret = real_mknod(reworked_path, mode, dev);
    LDFL_LOG_ERR(ret == 0, "real_mknod failed: pathname=%s, mode=%d, dev=%ld, errno=%d (%s)", reworked_path, mode, dev, errno, strerror(errno));
...
int statx(int dirfd, const char *restrict pathname, int flags, unsigned int mask, struct statx *restrict statxbuf) {
    ...
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "statx called: dirfd=%d, pathname=%s, flags=%d, mask=%u", dirfd, pathname, flags, mask);
    ...
    LDFL_LOG_ERR(ret == 0, "real_statx failed: dirfd=%d, pathname=%s, flags=%d, mask=%d, errno=%d (%s)", dirfd, reworked_path, flags, mask, errno,
                 strerror(errno));
...
int creat(const char *pathname, mode_t mode) {
    ...
    LDFL_LOG_ERR(ret == 0, "real_creat failed: pathname=%s, mode=%d, errno=%d (%s)", reworked_path, mode, errno, strerror(errno));
...
int renamex_np(const char *oldpath, const char *newpath, int flags) {
    RINIT;
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "renamex_np called: oldpath=%s, newpath=%s, flags=%d", oldpath, newpath, flags);
    ...
    LDFL_LOG_ERR(ret == 0, "real_renamex_np failed: oldpath=%s, newpath=%s, flags=%d, errno=%d (%s)", reworked_oldpath, reworked_newpath, flags,
                 errno, strerror(errno));
...
int renameatx_np(int olddirfd, const char *oldpath, int newdirfd, const char *newpath, int flags) {
    RINIT;
    uint64_t op_mask = LDFL_OP_NOOP | LDFL_OP_MAP | LDFL_OP_MEM_OPEN | LDFL_OP_STATIC | LDFL_OP_PERM | LDFL_OP_DENY;
    ldfl_setting.logger(LDFL_LOG_FN_CALL, LOG_DEBUG, "renameatx_np called: olddirfd=%d, oldpath=%s, newdirfd=%d, newpath=%s, flags=%d", olddirfd,
                        oldpath, newdirfd, newpath, flags);
